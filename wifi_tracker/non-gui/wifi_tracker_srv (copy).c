...
int measure_strength_ip(char ip[20])
{
...
main()
{
	...
	while(1)
	{
		for(i=0; i<no_of_ip; i++)
		{
			dist1 = measure_strength_ip(track_ip[i]);
			dist2 = send_rcv_from_helper(track_ip[i]);
			locate_and_record(i,dist1,dist2);
		}
		sleep(5);
	}
}
